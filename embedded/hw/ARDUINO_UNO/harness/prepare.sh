...
cp "$REPO/embedded/profiles/profile_core.c"   "$HERE/src/profile_core.c"
...
if ! git -C "$REPO" diff --quiet HEAD -- \
        ...
        embedded/profiles/profile_core.c \
...
cc -std=c99 -O2 -I"$HERE/include" \
    ...
    "$HERE/src/profile_core.c" \
    -o "$HOSTDIR/host_core"
HOST_LINE=$("$HOSTDIR/host_core")
echo "host reference: $HOST_LINE"
PIN_HASH=$(printf '%s\n' "$HOST_LINE" \
    | sed -n 's/^PROFILE core hash=0x\([0-9a-f]\{16\}\)$/\1/p')
if [ -z "$PIN_HASH" ]; then
...
fi
{
    ...
    printf '#define PINNED_PROFILE "core"\n'
